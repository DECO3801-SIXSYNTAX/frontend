...
function Protected({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    // Check Firebase authentication - cek user di localStorage (sudah disimpan saat login)
    const user = localStorage.getItem('user');
    const firebaseUid = localStorage.getItem('firebaseUid');
    
    setIsAuth(!!(user && firebaseUid));
    setIsChecking(false);
  }, []);
...
function RoleGuard({ children, allowedRoles }: { children: React.ReactNode; allowedRoles: string[] }) {
  ...
  // Get user from context OR localStorage (fallback during navigation)
  ...
  // Check if user exists (Firebase user sudah login)
  const firebaseUid = localStorage.getItem('firebaseUid');
  const isAuthenticated = user && firebaseUid;

  if (!isAuthenticated) {
    console.log('No auth - User:', !!user, 'FirebaseUID:', !!firebaseUid);
  ...
  if (allowedRoles.length > 0 && !allowedRoles.includes(userRole)) {
    ...
    if (userRole === 'planner') return <Navigate to="/planner/dashboard" replace />;
...
export default function App() {
  ...
  React.useEffect(() => {
    // Check for Firebase user (bukan Django JWT lagi)
    const user = localStorage.getItem('user');
    const firebaseUid = localStorage.getItem('firebaseUid');
      ...
      {(isAdminRoute || isVendorRoute) && (
        <>
          ...
          <div className="flex">
            ...
            {isVendorRoute && <DashboardSidebar />}
            <main className="flex-1 p-6">
              <Routes>
                ...
                <Route path="/vendor/seating/:eventId" element={<VendorOnly><SeatingView /></VendorOnly>} />
