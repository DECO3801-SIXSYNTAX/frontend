...
export class FloorPlanService {
  ...
  async getFloorPlanByEventId(eventId: string): Promise<FloorPlan | null> {
    try {
      console.log('🔍 [FloorPlanService] Getting floor plan for event:', eventId);
      
      // Get the metadata document at /events/{eventId}/floorplan/metadata
      const metadataRef = doc(db, 'events', eventId, 'floorplan', 'metadata');
      console.log('→ Metadata path:', `events/${eventId}/floorplan/metadata`);
      
      const metadataDoc = await getDoc(metadataRef);
      console.log('→ Metadata doc exists?', metadataDoc.exists());

      if (!metadataDoc.exists()) {
        console.warn('⚠️ No floor plan metadata found for event:', eventId);
        return null;
      }
      ...
      console.log('→ Elements path:', `events/${eventId}/floorplan/metadata/elements`);
      
      const elementsSnapshot = await getDocs(elementsRef);
      console.log('→ Elements found:', elementsSnapshot.docs.length);
      ...
      console.log('✅ [FloorPlanService] Floor plan loaded successfully');
      return floorPlan;
    } catch (error) {
      console.error('❌ [FloorPlanService] Error fetching floor plan:', error);
