...
export class DashboardService {
  ...
  async getEvents(): Promise<Event[]> {
    try {
      ...
      console.log('→ Fetching events from Firestore for user:', userId);
      
      // Get events dari Firestore
      ...
      console.log('✓ Fetched events from Firestore:', events.length);
      return events;
    } catch (error) {
  ...
  async createEvent(eventData: CreateEventForm, userId?: string): Promise<Event> {
    try {
      ...
      const newEvent = {
        ...
        collaborators: [], // ← TAMBAHKAN INI (array kosong untuk event baru)
  ...
  async getGuests(eventId?: string): Promise<Guest[]> {
    try {
      ...
      if (eventId) {
        // Get guests untuk event tertentu
        ...
        console.log('✓ Fetched guests for event:', eventId, guests.length);
        return guests;
      } else {
        ...
        console.log('✓ Fetched all guests:', allGuests.length);
        return allGuests;
      }
    } catch (error) {
  ...
  async updateGuest(eventId: string, guestId: string, updates: Partial<Guest>): Promise<Guest> {
    try {
      ...
      // Create updatedData dengan proper typing
      const updatedData: Record<string, any> = {
        ...updates,
        updatedAt: serverTimestamp(),
      };

      // Remove undefined values dengan proper type handling
  ...
  async addGuest(eventId: string, guestData: Partial<Guest>): Promise<Guest> {
    try {
      ...
      await this.logActivity({
        ...
        userName: auth.currentUser?.displayName || 'Current User',
        action: 'Added guest',
        details: guestData.name || 'New Guest',
        eventId: eventId,
        type: 'guest'
      });
