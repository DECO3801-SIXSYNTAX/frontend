...
import { authService, User } from "../services/UnifiedAuthService";
...
export default function SignIn() {
  ...
  useEffect(() => {
    ...
    if (token && user) {
      try {
        ...
        console.error('Failed to parse user data, clearing session');
        localStorage.clear();
      }
    }
  }, [navigate]);
  ...
  try {
    ...
    setCurrentUser(user);
    console.log('✓ Current user set in context:', user);

    // Show success message
    showMessage(`Welcome back, ${user.name}! Sign in successful.`, "success");

    // LANGSUNG navigate tanpa delay (hapus setTimeout)
    console.log('✓ Navigating to dashboard as:', user.role);
              ...
              >
                ...
                <GoogleButton
                  onSuccess={async (user) => {
                    // Google OAuth already handled by GoogleButton
                    // User is already authenticated and stored in localStorage
                    try {
                      ...
                      // Set user data from backend (already returned by GoogleButton)
                      setCurrentUser(user);
