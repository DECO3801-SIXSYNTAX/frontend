...
interface FloorPlan {
  id?: string;
  eventId: string;  // ← HAPUS tanda ? (jadi required)
...
const LayoutEditor: React.FC<LayoutEditorProps> = () => {
  ...
  useEffect(() => {
    const loadFloorPlan = async () => {
      ...
      try {
        ...
        const plan = await floorPlanService.getFloorPlanByEventId(eventId); // FIX: ganti dari getFloorPlan
        
        if (plan) {
          console.log('Floor plan loaded:', plan);
          setFloorPlanId(plan.id || null);
          setLayoutElements(plan.elements || []);
          setCanvasSize(plan.canvasSize || { width: 2000, height: 1500 });
          setTempCanvasSize(plan.canvasSize || { width: 2000, height: 1500 });
          setPixelsPerMeter(plan.pixelsPerMeter || 50);
          setRoomBoundary(plan.roomBoundary || null);
        } else {
          console.log('No existing floor plan found');
          setLayoutElements([]);
        }
      } catch (error) {
    ...
    const loadGuests = async () => {
      console.log('→ Starting loadGuests, eventId:', eventId);
      
      if (!eventId) {
        console.warn('⚠ No eventId, setting loadingGuests to false');
      ...
      try {
        ...
        console.log('✓ Loaded guests from Firebase:', firebaseGuests.length, 'guests');
        ...
        const convertedGuests: Guest[] = firebaseGuests.map(fbGuest => ({
          ...
          email: fbGuest.email,
          role: 'Guest' as Guest['role'],
          dietaryRestrictions: fbGuest.dietaryNeeds 
            ? fbGuest.dietaryNeeds.split(',').map(s => s.trim()).filter(Boolean)
            : [],
        ...
        console.log('✓ Converted guests:', convertedGuests.length);
        ...
        console.log('✓ Setting loadingGuests to false');
        setLoadingGuests(false);
      }
    };
  ...
  const handleSaveGuest = async () => {
    ...
    try {
      ...
      if (editingGuest) {
        // Update existing guest in Firestore
        if (eventId) {
          await guestService.updateGuest(eventId, editingGuest.id, {
            name: guestForm.name,
            email: guestForm.email,
            role: guestForm.role,
            dietaryNeeds: dietary.join(', '),
        ...
        setGuests(guests.map(g =>
          g.id === editingGuest.id
            ? { ...g, name: guestForm.name, email: guestForm.email, role: guestForm.role, dietaryRestrictions: dietary, accessibilityNeeds: accessibility }
            : g
        ));
      } else {
        ...
        if (eventId) {
          ...
          const createdGuest = await guestService.addGuest(eventId, newGuestData);
          
          console.log('✓ Guest added to Firestore:', createdGuest);

          // Add to local state with proper Guest interface
          ...
          alert('No event ID found. Cannot save guest.');
          return;
        }
      }
  ...
  const handleAssignGuest = async (guestId: string, elementId: string) => {
    ...
    try {
      ...
      // Update local state - element assignments
      const updatedElements = layoutElements.map(el => ({
        ...el,
        assignedGuests: el.id === elementId
          ? [...el.assignedGuests.filter(id => id !== guestId), guestId]
          : el.assignedGuests.filter(id => id !== guestId)
      }));
      
      setLayoutElements(updatedElements);

      // Save layout to Firestore with updated assignments
      if (floorPlanId) {
        const updatedPlan: FloorPlan = {
          id: floorPlanId,
          eventId: eventId, // eventId is guaranteed to be string here
        ...
        await floorPlanService.saveFloorPlan(updatedPlan);
        console.log('✓ Layout saved with guest assignment');
      }
      ...
      alert(`Failed to assign guest: ${error.message}`);
    }
  };
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4 border-t border-gray-200">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center space-x-2">
                  {selectedElement.config.icon ? (
                    React.createElement(selectedElement.config.icon, {
                      className: 'h-5 w-5',
                      style: { color: selectedElement.config.color }
                    })
                  ) : (
                    <Square className="h-5 w-5" style={{ color: selectedElement.config.color }} />
                  )}
                  <span className="text-sm text-gray-600">{selectedElement.config.label}</span>
                </div>
              </div>
              <p className="text-xs text-gray-500">{selectedElement.config.description || 'Custom element'}</p>
            </div>
