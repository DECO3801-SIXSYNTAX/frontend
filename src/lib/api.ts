...
export const api = {
  ...
  listEvents: async (): Promise<EventItem[]> => {
    // Try admin endpoint first; if 403, fall back to planner/events endpoint.
    ...
    if (e.status === 403) throw new Error('Forbidden: your account lacks permission to view events. Ask an admin to grant planner/admin access.');
